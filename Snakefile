...
rule rank_experiments:
    '''
    Performs statistical test on list of PC components. Output table with LABS and PVALS
    '''
    resources:
        mem_ram=32
        # threads=8 # for slow algorithms
    input:
    ...
    message:
        ...
        " Max PC components for 1 dataset: {wildcards.max_comp} \n"
    log:
        "logs/{organism}/{platform}/{test}/"
        "{n_genes}_{scale}_{max_comp}_{var_threshold}/"
        "{geneset_name}.log"
    conda: "envs/fgsea.yaml"
    shell:
        "Rscript scripts/R/rank_experiments.R {input.pc_list} {input.geneset} {wildcards.test} {output}"
        " > {log} 2>&1"

rule prepare_result:
    input:
        ks_results=rules.rank_experiments.output,
        gse_df="out/{organism}/{platform}/sm_metadata/gse.tsv"
    output:
        "out/{organism}/{platform}/{test}/{n_genes}_{scale}_{max_comp}_{var_threshold}/"
        "prepared/{geneset_name}.tsv"
    message:
        ...
        " Max PC components for 1 dataset: {wildcards.max_comp} \n"
    log:
        "logs/{organism}/{platform}/{test}_prepare_results/"
        "{n_genes}_{scale}_{max_comp}_{var_threshold}/"
        "{geneset_name}.log"
    conda: "envs/r_scripts.yaml"
    shell:
        "Rscript scripts/R/prepare_results.R {input.ks_results} {input.gse_df} {output}"
        " > {log} 2>&1"
