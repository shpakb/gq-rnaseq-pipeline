...
def get_filtered_gsm_files_for_gse(wildcards, min_gsm=None, max_gsm=None,
                                   gpl_df_file=None, # if to be filtered by supported gpl
                                   srr_df_file=None, min_spots=None, max_spots=None, # if to be filtered by the number of spots
                                   gsm_qc_file=None, min_exp_genes=None):
    ...
    if wildcards.organism not in glob_gse_gsm_df:
        glob_gse_gsm_df[wildcards.organism] = \
            get_filtered_gse_gsm_map(f"out/{wildcards.organism}/seq/sm_metadata/gsm.tsv", wildcards.organism,
                gpl_df_file=gpl_df_file)
...
rule srr_to_gsm:
    resources:
        mem_ram=1
    input:
        srr_files=get_srr_files_for_gsm,
...
def get_filtered_gsm_files_prequant(wildcards, **kwargs):
    ...
    return \
        expand("out/{organism}/{platform}/gsms/{gsm}.tsv",
            organism=wildcards.organism, platform=wildcards.platform, gsm=gsm_list)

rule aggregate_gsm_qc_df:
    input:
        lambda wildcards:
            get_filtered_gsm_files_prequant(wildcards,
...
rule gsm_to_gse:
    input:
        gsm_files=get_gsm_files_for_gse,
...
#     print(gse_files"Getting list of GSE to be aggregated")
...
def get_filtered_tags(wildcards, min_gsm=int, max_gsm=int, min_genes=int,
    ...
    # todo
        ...
        gsm_gse_df_file = \
            filter_gsm_df(
                gsm_df_file, organism, min_gsm=2,
                max_gsm=200, min_spots=None,
                max_spots=None, srr_df_file=None)
