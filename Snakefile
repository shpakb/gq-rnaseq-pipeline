...
rule all:
    input:
        ...
        expand("out/mm/seq/gses_raw_counts/{gse}.tsv", gse=["GSE89477", "GSE89263"])
        # expand("out/hs/seq/gses_raw_counts/{gse}.tsv", gse=["GSE51254", "GSE69790", "GSE124989", "GSE125300", "GSE132525",
        #                                        "GSE132172", "GSE130473", "GSE130289", "GSE127298"])
...
def filter_gsm_df(gsm_df_file, organism,
    ...
    """
    organism = {"hs": "Homo sapiens", "mm": "Mus musculus", "rn": "Rattus norvegicus"}[organism]
    gsm_df = pd.read_csv(gsm_df_file, sep='\t')
    gsm_df = gsm_df[gsm_df["ORGANISM"] == organism]
    ...
    if min_gsm:
        gse_counts_df = gsm_df.groupby('GSE')['GSE'].transform('count')
    ...
    #         gsm_df = filter_gsm_df(f"out/{wildcards.organism}/seq/sm_metadata/gsm.tsv", wildcards.organism)
...
def get_gsm_for_gse(wildcards):
    print(f"Getting filtered GSM files for {wildcards.gse}")
    global glob_gse_gsm_df

    if wildcards.organism not in glob_gse_gsm_df:
        glob_gse_gsm_df[wildcards.organism] = \
            filter_gsm_df(f"out/{wildcards.organism}/seq/sm_metadata/gsm.tsv", wildcards.organism)

    gsm_gse_df = glob_gse_gsm_df[wildcards.organism]
...
rule gsm_to_gse_raw_counts:
    ...
    input:
        ...
        gene_mapping="input/{organism}/seq/ensembl_symbol_entrez.tsv"
    output:
        gse="out/{organism}/seq/gses_raw_counts/{gse}.tsv"
    log: "logs/{organism}/gsm_to_gse_raw_counts/{gse}.log"
    message: "Aggregating {wildcards.gse} ({wildcards.organism})..."
    shadow: "shallow"
    conda: "envs/r_scripts.yaml"
    shell:
