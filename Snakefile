    ...
    if wildcards.organism not in glob_postquant_gse_gsm_map:
        glob_postquant_gse_gsm_map[wildcards.organism] = \
            get_filtered_gse_gsm_map(
                ...
                min_spots=int(config["min_spots"][wildcards.organism]),
                max_spots=int(config["max_spots"][wildcards.organism]),
                gsm_qc_df=rules.aggregate_gsm_qc_df.output
        )
...
checkpoint aggregate_gse_qc_df:
    ...
    #             max_spots=int(config["max_spots"][wildcards.organism]))
    output:
        gsm_qc_df="out/{organism}/{platform}/gse_cpm_qc.tsv"
    run:
        print("Aggregating GSE qc df...")
        gses_dir="output/{wildcards.organism}/seq/gses_cpm"
        gse_files = [f for f in listdir(gses_dir) if isfile(join(gses_dir, f))]
        n_gsm_list=\
            [
...
def get_filtered_exp_mat_files(wildcards, min_gsm=int, max_gsm=int, min_genes=int,
    ...
    if wildcards.platform=="chip":
        ...
        gse_qc_df = checkpoints.aggregate_gse_qc_df.get(**wildcards).output
